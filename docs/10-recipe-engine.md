...
## 6.2 AeroPress エスプレッソ風の特別対応

同じ AeroPress でも「ドリップ寄り」（`aeropress`）と「エスプレッソ風の少量濃縮ショット」
（`aeropress-espresso`）は淹れ方が別物のため、taste ベクトルで切り替えるのではなく
**別の器具として登録**し、ウィザードの器具選択に両方を並べる（docs/06 S02）。

```
steps = [
  { kind: 'pour', atSec: 0, toWaterG: waterG },  // 蒸らしなし・全量を一度に注ぐ
  { kind: 'stir', atSec: 5, method: 'spoon' },
  { kind: 'press', atSec: 15 + steepSec, durationSec: 20 },
]
steepSec = clamp(40 + body*10 - clarity*8, 20, 75)
```

- `ratioRange:[2,3]` という低い値により `computeRatio()`（§5-(3)）の比率レンジ補正が
  自動的に doseG を押し上げる。TDS/EY の目標値自体は他ドリッパーと同じ一般式のままでよく、
  coldDrip のような湯温モデルの分岐は不要
- 少量ショット(40〜90ml)を選べるよう `brewInputSchema.targetVolumeMl` の下限を 100ml→30ml に緩和した
